...
export class PopupService {
  ...
  public error(title: string, text: string) {
    this.windowService
      .openApplication(PopupComponent, new PopupParams(PopupType.Error, text, title), W2kWindowFrameComponent)
      .ifPresent(id => {
        this.centerPopup(id)
  ...
  public info(title: string, text: string) {
    this.windowService
      .openApplication(PopupComponent, new PopupParams(PopupType.Info, text, title), W2kWindowFrameComponent)
      .ifPresent(id => {
        this.centerPopup(id)
  ...
  private centerPopup(id: number) {
    setTimeout(() => {
      this.windowService
        .getDisplaySize()
        .ifPresent(([x,y]) => this.windowService.setPosition(id, x/2, y/2, true))
    })
  }
}
