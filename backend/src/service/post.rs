...
impl PostService {
    ...
    pub async fn edit_post(&self, author_id: i64, post_id: i64, content: &str) -> SysRes<()> {
        let q = query(
            r#"
    ...
    pub async fn delete_post(&self, author_id: i64, post_id: i64) -> SysRes<()> {
        let q = query(
            r#"
            match (u:Profile{id: $user_id})-[r:POSTED]->(p:Post)
            where id(p)=$post_id
    ...
    pub async fn create_post(&self, author_id: i64, content: &str) -> SysRes<Post> {
    ...
    pub async fn get_posts_for_user(&self, user_id: i64) -> SysRes<Vec<Post>> {
        let q = query(
            r#"
        ...
        let mut res = self.neo4j.execute(q).await?;
...
impl TryFrom<Row> for Post {
    ...
    fn try_from(value: Row) -> SysRes<Self> {
        ...
        Ok(Self {
            ...
            content: post
                .get("content")
                .map_err(|_| Error::Neo4jInvalidNode(post.id()))?,
            date: relation
                .get("date")
                .map_err(|_| Error::Neo4jInvalidNode(relation.id()))?,
        })
    }
}
